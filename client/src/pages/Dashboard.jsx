...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchMyTrips = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/trips/creator/${currentUserId}`);
        setMyTrips(response.data);
        setLoading(false);
      } catch (error) {
        console.error("Dashboard route not ready:", error.message);
        // SMART FALLBACK: Simulated trip with applicants so you can see the UI!
        setMyTrips([
          {
            _id: 'trip-101',
  ...
  const handleDecision = async (tripId, applicantId, decision) => {
    try {
      ...
      // await axios.post(`http://localhost:5000/api/trips/${tripId}/manage-applicant`, { applicantId, decision });
      
      // For now, we update the UI locally to simulate the action
      setMyTrips(prevTrips => prevTrips.map(trip => {
        if (trip._id === tripId) {
          const applicantToMove = trip.applicants.find(a => a._id === applicantId);
          return {
            ...trip,
            applicants: trip.applicants.filter(a => a._id !== applicantId), // Remove from applicants
            approvedMembers: decision === 'approve' 
              ? [...(trip.approvedMembers || []), applicantToMove] // Add to approved if accepted
      ...
      alert(`Applicant successfully ${decision}d! (Simulated)`);
    } catch (error) {
...
{/* Trip Header */}
              ...
              <div style={{ padding: '1.5rem' }}>
                <h3 style={{ margin: '0 0 1rem 0', color: '#334155', fontSize: '1.1rem' }}>Pending Applications ({trip.applicants.length})</h3>
                ...
                {trip.applicants.length === 0 ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    {trip.applicants.map(applicant => (
                      <div key={applicant._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', background: '#f1f5f9', borderRadius: '8px' }}>
                        
                        <div>
                          <h4 style={{ margin: '0 0 0.25rem 0', color: '#1e293b' }}>{applicant.name}</h4>
